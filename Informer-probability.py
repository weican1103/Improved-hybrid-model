...
def cal_eval(y_real, y_pred):
    ...
    return df_eval
df = pd.read_csv('data\\2019.csv') # 将你所需要的数据放到data文件夹，然后只要改成你自己的文件名
...
num_test = 110
batch_size = 64
window = 10  # 模型输入序列长度 过去30天的数据
length_size = 1  # 预测结果的序列长度  预测未来1天

data_train = data_inverse[:-num_test, :]  # 读取目标数据，第一列记为0：1，后面以此类推, 训练集和验证集，如果是多维输入的话最后一列为目标列数据
data_train_mark = data_stamp[:-num_test, :]
data_test = data_inverse[-num_test:, :]  # 这里把训练集和测试集分开了，也可以换成两个csv文件
data_test_mark = data_stamp[-num_test:, :]
...
class QuantileLoss(nn.Module):
    def __init__(self, quantiles):
        super().__init__()
        self.quantiles = torch.tensor(quantiles).float().to(device)
...
model_type = 'Informer'  # 'Autoformer'
net1 = Informer.Model(config).to(device)

criterion1 = QuantileLoss([Q_1])  # 损失函数
optimizer1 = optim.Adam(net1.parameters(), lr=learning_rate)  # 优化器
scheduler1 = ReduceLROnPlateau(optimizer1, mode='min', factor=0.1, patience=scheduler_patience, verbose=True)  # 学习率调整策略

trained_model1, train_loss, final_epoch = model_train(net1, train_loader, length_size, optimizer1, criterion1, num_epochs, device, print_train=True)

net2= Informer.Model(config).to(device)

criterion2 = QuantileLoss([Q_2])  # 损失函数
optimizer2 = optim.Adam(net2.parameters(), lr=learning_rate)  # 优化器
scheduler2 = ReduceLROnPlateau(optimizer2, mode='min', factor=0.1, patience=scheduler_patience, verbose=True)  # 学习率调整策略

trained_model2, train_loss, final_epoch = model_train(net2, train_loader, length_size, optimizer2, criterion2, num_epochs, device, print_train=True)

trained_model1.eval()  # 模型转换为验证模式
trained_model2.eval()  # 模型转换为验证模式
# 预测并调整维度
pred1 = trained_model1(x_test.to(device), x_test_mark.to(device), y_test.to(device), y_test_mark.to(device))
pred2 = trained_model2(x_test.to(device), x_test_mark.to(device), y_test.to(device), y_test_mark.to(device))
...
print("Shape of pred before adjustment:", pred1.shape)
print("Shape of pred before adjustment:", pred2.shape)

# 可能需要调整pred和true的维度，使其变为二维数组
true = true[:, :, -1]
pred1 = pred1[:, :, -1]  # 假设需要将pred调整为二维数组，去掉最后一维
pred2 = pred2[:, :, -1]  # 假设需要将pred调整为二维数组，去掉最后一维
# true =np.array(true)
# 假设需要将true调整为二维数组

print("Shape of pred after adjustment:", pred1.shape)
print("Shape of pred after adjustment:", pred2.shape)
...
pred_uninverse1 = scaler.inverse_transform(pred1[:, -1:]).reshape(-1)    #如果是多步预测， 选取最后一列
pred_uninverse2 = scaler.inverse_transform(pred2[:, -1:]).reshape(-1)    #如果是多步预测， 选取最后一列
true_uninverse = scaler.inverse_transform(true[:, -1:]).reshape(-1)

Real, output_test_pred11, output_test_pred22 = true_uninverse, pred_uninverse1, pred_uninverse2

PINC = int((1 - 2 * Q_2)*100)
plt.figure()
plt.plot(output_test_pred11, color='blue')
plt.plot(output_test_pred22, color='blue')
plt.fill_between(range(len(output_test_pred11)), output_test_pred11, output_test_pred22, alpha=0.5, label="{}% Interval".format(PINC))
plt.plot(Real, 'k', marker='o', linewidth=2.5,label='Real')
plt.setp(plt.gca().lines[-1], markersize=4, markeredgecolor='k', markeredgewidth=1.5)
plt.legend()
plt.show()

# 计算PICP
a = 0
for i in range (len(output_test_pred22)):
    if output_test_pred22[i] <= Real[i] <= output_test_pred11[i]:
        a = a+1

PICP = a/len(output_test_pred22)
print('在',1-2*Q_2,'的置信区间上，实际得到的区间覆盖率为：',PICP)

# 计算MPIW
diff = output_test_pred11 - output_test_pred22
diff = diff.reshape(-1,1)
MPIW = np.mean(diff)
MPIW = np.round(MPIW, decimals=2)
print('MPIW (Mean Prediction Interval Width):', MPIW)

# 计算AIS
record = []
for i in range(len(diff)):
    if Real[i] <= output_test_pred22[i]:
        AIS = -0.02 * diff[i] - 4 * (output_test_pred22[i] - Real[i])
    elif output_test_pred22[i] <= Real[i] <= output_test_pred11[i]:
        AIS = -0.02 * diff[i]
    elif Real[i] >= output_test_pred11[i]:
        AIS = -0.02 * diff[i] - 4 * (Real[i] - output_test_pred11[i])

    record.append(AIS)
record = np.array(record).reshape(-1,1)
AIS = np.mean(record)
AIS = np.round(AIS, decimals=2)
print('AIS (Average Interval Score):', AIS)


# 将真实值和预测值合并为一个 DataFrame
result_df = pd.DataFrame({'真实值': Real.flatten(), '预测值Q1': output_test_pred11.flatten(),'预测值Q2': output_test_pred22.flatten()})
# 保存 DataFrame 到一个 CSV 文件
result_df.to_csv('实验二-data4-informer-概率预测95%.csv', index=False, encoding='utf-8')
# 打印保存成功的消息
